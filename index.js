...
function convertGifToSticker(buffer) {
    return new Promise((resolve, reject) => {
        const ts = Date.now();
        const inputPath = path.join(__dirname, `temp_gif_input_${ts}.gif`);
        const outputPath = path.join(__dirname, `temp_gif_output_${ts}.webp`);
        ...
        ffmpeg(inputPath)
            .outputOptions([
                '-vcodec', 'libwebp',
                '-vf', "scale='min(512,iw)':'min(512,ih)':force_original_aspect_ratio=decrease,pad=512:512:(512-iw)/2:(512-ih)/2:color=black@0,fps=15",
                '-loop', '0',
                '-preset', 'default',
                '-an',
                '-vsync', '0',
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        // Normalisasi sender: pastikan hanya angka tanpa +, spasi, atau prefix
        const senderClean = sender.replace(/[^0-9]/g, '');
        const isOwner = OWNER_NUMBERS.some(n => n.replace(/[^0-9]/g, '') === senderClean);
        // Debug (hapus baris ini setelah dipastikan bekerja)
        if (isCmd) console.log(`[DEBUG] sender: "${sender}" | senderClean: "${senderClean}" | isOwner: ${isOwner}`);
        ...
        if (isGroup) {
            try {
                ...
                // Normalisasi ID: hapus device suffix (misal :12) sebelum @
                const normalize = (jid) => jid ? jid.replace(/:[0-9]+@/, '@') : '';
                ...
                isAdmins = participants.some(p => {
                    ...
                    return (pid === normalize(rawSender) || pid === senderJid) && (p.admin === 'admin' || p.admin === 'superadmin');
                });
                isBotAdmins = participants.some(p => normalize(p.id) === botId && (p.admin === 'admin' || p.admin === 'superadmin'));
            } catch (e) { }
        }
